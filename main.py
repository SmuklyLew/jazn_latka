...
def _prepare_chatgpt_daemon_presentation(
    *,
    ...
    existing_host_bridge = _as_dict(result.get("chatgpt_host_bridge"))
    if user_text_binding_error is None:
        attach_chatgpt_host_contract(
            result,
            config=cfg,
        ...
        if existing_host_bridge:
            result["chatgpt_host_bridge"] = existing_host_bridge
        else:

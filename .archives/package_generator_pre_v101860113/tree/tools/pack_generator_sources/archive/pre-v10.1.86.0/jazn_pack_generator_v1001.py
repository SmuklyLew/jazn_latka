...
GENERATOR_TITLE = "Naprawiony generator dystrybucji Jaźni"
SETTINGS_SCHEMA = "jazn_pack_generator_settings/v10.0.1"
...
def _settings_path() -> Path:
    if os.name == "nt":
        ...
        return base / "Latka" / "JaznPackGenerator" / "settings-v10.0.1.json"
    return Path(os.environ.get("XDG_CONFIG_HOME") or (Path.home() / ".config")) / "latka" / "jazn-pack-generator-v10.0.1.json"
...
def materialize_native_dependency_bundle(
    ...
    *,
    ...
    target = resolve_distribution_target_alias(target_alias)
    python_minor = normalize_distribution_python_version(python_version)
    if target != current_distribution_target_alias():
        raise RuntimeError("Cross-target dependency materialization is forbidden. Build on the native runner.")
    ...
    *,
    ...
    if plan["dependencies"]:
        ...
        if bundle is None and materialize_dependencies:
            bundle = materialize_native_dependency_bundle(source_root, target_alias=target, python_version=python_minor)
        if bundle is None:
            raise PackError(
                f"Brak zweryfikowanego dependency bundle dla {target}/py{python_minor}. "
                "Wskaż bundle albo włącz natywne pobranie zależności."
            )
    env = dict(os.environ)
    env["PYTHONPATH"] = str(source_root) + (os.pathsep + env["PYTHONPATH"] if env.get("PYTHONPATH") else "")
    command = [
...
def distribution_request_plan(*, content: str, layout: str, archive_format: str) -> dict[str, Any]:
    ...
    return {
        "schema_version": "jazn_pack_generator_request_plan/v1",
        "generator_version": GENERATOR_VERSION,
...
def run_pack_request(
    *,
    ...
    plan = distribution_request_plan(content=content, layout=layout, archive_format=archive_format)
    ...
    work_root = Path(tempfile.mkdtemp(prefix=".jazn-pack-v1001-", dir=str(output_root)))
    ...
    try:
        ...
        public_report = {
            ...
            "truth_boundary": (
                "Generator v10.0.1 zachowuje kanoniczne artefakty package_distribution wewnątrz warstwy transportowej. "

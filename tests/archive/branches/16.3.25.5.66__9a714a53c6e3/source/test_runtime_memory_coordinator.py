...
from latka_jazn.memory.runtime_memory import RuntimeMemoryCoordinator, RuntimeMemoryWriteContext
...
def test_rejected_candidate_does_not_create_database(tmp_path: Path) -> None:
    database = tmp_path / "runtime-transactional.sqlite3"
    coordinator = RuntimeMemoryCoordinator(database, classifier=FakeClassifier(accepted=False))
...
def test_accepted_turn_writes_only_l1_l2_and_outbox(tmp_path: Path) -> None:
    database = tmp_path / "runtime-transactional.sqlite3"
    coordinator = RuntimeMemoryCoordinator(database, classifier=FakeClassifier())

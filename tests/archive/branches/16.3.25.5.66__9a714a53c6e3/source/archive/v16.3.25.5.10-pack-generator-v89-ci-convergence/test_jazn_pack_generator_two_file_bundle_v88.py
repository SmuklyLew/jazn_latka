...
def test_v88_generator_has_no_legacy_helper_files() -> None:
    for filename in LEGACY_HELPERS:
        assert not (ROOT / "tools" / filename).exists(), filename


def test_v88_generator_uses_bundled_modules_without_static_legacy_imports() -> None:
...
def test_v88_generator_imports_from_two_file_bundle() -> None:

...
def test_release_workflow_does_not_repeat_full_and_targeted_suites() -> None:
    text = _read("release-hardening.yml")
    ...
    assert "Turn atomicity and timeout regressions" not in text
...
def test_upload_artifact_is_pinned_and_failure_only() -> None:
    ...
    assert refs == [UPLOAD_ARTIFACT_V701_SHA] * 3
    assert all(re.fullmatch(r"[0-9a-f]{40}", ref) for ref in refs)
    assert text.count("if: failure()") == 3

...
async def test_http_gateway_exposes_redacted_runtime_and_memory_resources(tmp_path: Path) -> None:
    ...
    runtime_payload = json.loads(runtime.contents[0].text)
    memory_payload = json.loads(memory.contents[0].text)
...
async def test_http_gateway_task_resource_reads_same_durable_registry(tmp_path: Path) -> None:
    task = McpTaskStore(tmp_path).create_or_get(
        daemon_request_id="req-resource-v81",
        request_id="req-resource-v81",
        turn_id="turn-resource-v81",
        trace_id="trace-resource-v81",
    )
    gateway = _gateway(tmp_path)
    async with Client(gateway.mcp, raise_exceptions=True) as client:
    ...
    assert payload["lineage"]["daemon_request_id"] == "req-resource-v81"
    assert payload["lineage"]["turn_id"] == "turn-resource-v81"

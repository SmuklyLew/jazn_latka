...
def test_transactional_memory_diagnostic_reports_missing_then_ready_without_writes(tmp_path: Path) -> None:
    ...
    missing = diagnostics._transactional_memory_status(cfg)
    ...
    ready = diagnostics._transactional_memory_status(cfg)
...
def test_status_and_doctor_expose_separate_transactional_memory_subsystem(tmp_path: Path, monkeypatch) -> None:
    ...
    assert doctor["readiness"]["transactional_memory_ready"] is True
    assert doctor["live_evidence"]["transactional_memory_ready"] is True
    assert doctor["subsystems"]["memory"]["transactional_tier"]["ready"] is True

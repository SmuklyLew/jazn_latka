...
# -*- coding: utf-8 -*-
"""
Jaźń / Łatka — Restore Memory v24.0.1
...
Nie duplikuje transakcji, schematów ani klasyfikatorów. Zapewnia:
  ...
  - plan bez zapisu, jawne potwierdzenie restore i raportowanie,
  - wybór wielu eksportów ChatGPT oraz dziennika,
  - tryb developerski poza repo albo jawny restore systemowy,
...
Przykłady:
  py -X utf8 tools/restore_memory.py
  py -X utf8 tools/restore_memory.py --text-ui
  py -X utf8 tools/restore_memory.py --config D:\\.AI\\restore_memory_test_03.json
  py -X utf8 tools/restore_memory.py --no-ui --plan-only --all-discovered
  py -X utf8 tools/restore_memory.py --version
"""
...
TOOL_VERSION = "24.0.1"
TOOL_SCHEMA = "jazn_memory_restore_tool/v24"
CONFIG_NAME = "restore_memory_v24.json"
UI_CANCEL = "__JAZN_RESTORE_UI_CANCEL__"
UI_EXIT = "__JAZN_RESTORE_UI_EXIT__"
...
class RestoreToolError(RuntimeError):
...
def _detail_width() -> int:
    try:
        columns = shutil.get_terminal_size((110, 32)).columns
    except Exception:
        columns = 110
    preferred = int(round(columns * 0.28))
    return max(28, min(58, preferred, max(28, columns - 52)))
...
def _copy_windows_clipboard(text: str) -> None:
    ...
    try:
        if not user32.OpenClipboard(None):
            raise RestoreToolError("Nie można otworzyć schowka systemowego.")
        clipboard_open = True
        if not user32.EmptyClipboard():
            raise RestoreToolError("Nie można wyczyścić schowka systemowego.")
        ...
        if not user32.SetClipboardData(CF_UNICODETEXT, handle):
            raise RestoreToolError("Nie można zapisać tekstu do schowka systemowego.")
...
def copy_system_clipboard(text: str) -> None:
    ...
    try:
        ...
        raise RestoreToolError(f"Nie udało się skopiować tekstu do schowka: {exc}") from exc
...
def save_plan_report(state: "ToolState", plan: MemoryRestorePlan, text: str) -> tuple[Path, Path]:
    ...
    output_root = target_root.parent / "restore_memory_plans"
    stamp = time.strftime("%Y%m%dT%H%M%S", time.gmtime())
    target_name = target_root.name or "memory_restore"
...
def default_config_path() -> Path:
    override = os.environ.get("JAZN_RESTORE_CONFIG")
    if override:
...
def load_state(path: Path | None) -> ToolState:
    config = (path or default_config_path()).expanduser().resolve()
    if not config.is_file():
        return ToolState(default_settings(), ui_mode="cursor" if HAS_PROMPT_TOOLKIT else "text", config_path=config)
    payload = json.loads(config.read_text(encoding="utf-8-sig"))
    if not isinstance(payload, dict):
        raise RestoreToolError(f"Konfiguracja nie jest obiektem JSON: {config}")
    raw_settings = payload.get("settings", payload)
    if not isinstance(raw_settings, dict):
        raise RestoreToolError(f"Brak obiektu settings w konfiguracji: {config}")
    ...
    state = ToolState(settings, selected, ui_mode=ui_mode, config_path=config)
    state.normalize()
    return state
...
def cursor_select(
    ...
    *,
    ...
    if not _cursor_ready():
        raise RestoreToolError("Interfejs kursorowy wymaga kompletnego prompt_toolkit.")
    ...
    def mouse_handler(row_index: int) -> Callable[[Any], object]:
        def handle(event: Any) -> object:
            ...
            if _is_right_up(event):
                ...
                return None
            if MouseEventType is not None and event.event_type == MouseEventType.MOUSE_MOVE:
                if index != row_index:
                    index = row_index
                    app.invalidate()
                return None
        ...
        layout=Layout(
            HSplit(
                [
                    ...
                    VSplit(
                        [
                            Window(content=menu, width=58, style="class:menu", wrap_lines=False),
                            Window(width=1, char="│", style="class:border"),
                            Window(
                                content=detail,
                                width=lambda: Dimension.exact(_detail_width()),
                                style="class:panel",
                                wrap_lines=True,
                            ),
                        ],
        ...
        layout=Layout(
            HSplit(
                [
                    ...
                    VSplit(
                        [
                            Window(content=FormattedTextControl(render_list, focusable=True), width=72, style="class:menu", wrap_lines=False),
                            Window(width=1, char="│", style="class:border"),
                            Window(
                                content=FormattedTextControl(render_detail),
                                width=lambda: Dimension.exact(_detail_width()),
                                style="class:panel",
                                wrap_lines=True,
                            ),
...
class ProgressScreen:
    def __init__(self, title: str, *, echo: bool = False):
        self.title = title
        self.echo = bool(echo)
    ...
    def run(self, worker: Callable[[], dict[str, Any]]) -> dict[str, Any]:
        if not _cursor_ready():
            def printing_callback(payload: dict[str, Any]) -> None:
                self.callback(payload)
                print(self.lines[-1], flush=True)
            # caller installs callback separately; this branch only runs worker
            return worker()
        ...
        def _ignore_exit(event: Any) -> None:
            # Nie przerywamy procesu w połowie transakcji przez sam interfejs.
            # Ctrl+C/KeyboardInterrupt obsługuje warstwa wykonawcza.
            return None

        header = FormattedTextControl(
            [
                ("class:header.title", f"  {self.title}"),
                ("class:header.subtitle", "  •  nie zamykaj terminala podczas transakcji"),
            ]
        )
            ...
            layout=Layout(
                HSplit(
                    [
                        Window(height=2, content=header, style="class:header"),
                        Window(height=1, char="─", style="class:border"),
                        Window(content=FormattedTextControl(render), style="class:panel", wrap_lines=True),
                        Window(height=1, char="─", style="class:border"),
                        Window(height=1, content=FormattedTextControl([("class:footer.text", " Trwa bezpieczny restore. Po zakończeniu pojawi się raport. ")]), style="class:footer"),
                    ]
                )
            ),
            key_bindings=keys,
        ...
        thread = threading.Thread(target=target, name="memory-restore-worker", daemon=False)
        thread.start()
        self.app.run()
        thread.join()
        self.app = None
        if self.error is not None:
            raise self.error
        return self.result or {"ok": False, "error": "worker_returned_no_result"}
...
def _main_details(state: ToolState) -> list[str]:
    ...
    return [
        ...
        "Pokazuje wynik ostatniego planu lub restore w bieżącej sesji.",
...
def select_sources(state: ToolState) -> None:
    ...
    if state.ui_mode == "cursor":
        selected = cursor_multi_select("ŹRÓDŁA RESTORE", items, state.selected_paths)
        if selected is None:
            return
...
def add_external_source(state: ToolState) -> None:
    ...
    if not path.is_file():
        raise RestoreToolError(f"Plik nie istnieje: {path}")
...
SETTING_SPECS = [
    ...
    ("continue_on_error", "Kontynuuj po błędzie", "Niebezpieczne w pierwszym pełnym restore; zalecane OFF."),
...
def build_plan(state: ToolState) -> MemoryRestorePlan:
    state.normalize()
    if not state.selected_paths:
        raise RestoreToolError("Nie wybrano żadnych źródeł.")
    progress = ProgressScreen("PLAN RESTORE", echo=state.ui_mode != "cursor")
...
def _plan_text(plan: MemoryRestorePlan) -> str:
    ...
    lines = [
        f"RESTORE MEMORY v{TOOL_VERSION}",
        "",
        f"OK: {payload.get('ok')}",
...
def show_plan(state: ToolState) -> None:
    ...
    if state.ui_mode == "cursor":
        cursor_view_text(
            "PLAN RESTORE — BEZ ZAPISU",
            text,
            subtitle="sprawdź wszystkie źródła i preflight",
...
def _confirm_restore(state: ToolState, plan: MemoryRestorePlan) -> str | None:
    ...
    if value is None:
        return None
    if value != expected:
        raise RestoreToolError("Nieprawidłowy token potwierdzenia. Restore anulowany.")
    return value


def run_restore(state: ToolState) -> None:
    plan = state.last_plan or build_plan(state)
    if not plan.ok:
        raise RestoreToolError("Plan jest zablokowany. Otwórz plan i usuń błędy preflight.")
    token = _confirm_restore(state, plan)
    if token is None:
        return
    progress = ProgressScreen("PRZYWRACANIE PAMIĘCI ŁATKI", echo=state.ui_mode != "cursor")
    orchestrator = MemoryRestoreOrchestrator(
        state.settings,
        tool_root=ROOT,
        callback=progress.callback,
    ...
    if not result.get("ok"):
        raise RestoreToolError("Restore zakończył się błędem. Szczegóły znajdują się w raporcie.")
...
def show_last_report(state: ToolState) -> None:
    if state.last_result is not None:
        ...
        title = "OSTATNI WYNIK RESTORE"
    elif state.last_plan is not None:
        text = _plan_text(state.last_plan)
        title = "OSTATNI PLAN"
    else:
        text = "Brak planu lub restore w bieżącej sesji."
        title = "BRAK RAPORTU"
    if state.ui_mode == "cursor":
...
def self_test(state: ToolState) -> dict[str, Any]:
    ...
    checks.append({"name": "tool_version", "ok": TOOL_VERSION == "24.0.0", "value": TOOL_VERSION})
    ...
    if state.ui_mode == "cursor":
        cursor_view_text("AUTOTEST RESTORE MEMORY", text)
    else:
        print(text)
    return report
...
def edit_mode(state: ToolState) -> None:
    ...
    choice = choose(state, "TRYB RESTORE", rows, details=details, selected=0 if state.settings.mode == "developer" else 1)
    if choice is None:
        return
    payload = state.settings.to_dict()
...
def toggle_ui(state: ToolState) -> None:
    if state.ui_mode == "cursor":
        ...
        raise RestoreToolError("Brak prompt_toolkit; dostępny jest tylko interfejs tekstowy.")
    state.dirty = True
...
def run_interactive(state: ToolState) -> int:
    ...
    while True:
        ...
        choice = choose(
            state,
            f"NARZĘDZIA JAŹNI — RESTORE MEMORY v{TOOL_VERSION}",
            rows,
        ...
        try:
            ...
            return 0
        except (RestoreToolError, ValueError, OSError, json.JSONDecodeError) as exc:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="restore_memory",
        description=f"Restore Memory v{TOOL_VERSION}: kontrolowana odbudowa pięciu baz pamięci Jaźni.",
    )

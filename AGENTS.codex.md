...
- JavaScript jest opcjonalną capability prezentacyjną/narzędziową: Pythonowy `run.py` pozostaje cienkim starterem użytkownika, a `main.py` kanonicznym centralnym operatorem. Browser ESM może należeć do `latka_jazn/resources/runtime_console/`, natomiast Node/ESM testy i tooling pozostają pod `tools/javascript/`. JavaScript nie może stać się właścicielem lifecycle, pamięci/SQLite, routingu, identity ani turn/finalization authority. Brak Node.js nie może blokować runtime ani Runtime Console; bieżącą linią testowaną w CI jest Node.js 24 LTS. Repo śledzi `package-lock.json`, instaluje stan CI przez `npm ci` i nie commituj `node_modules/`.

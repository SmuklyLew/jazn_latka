...
`tests/test_v1601_memory_transport_generator.py` celowo używa wersji `v15.0.3.222-RUN HOTFIX` jako historycznej próbki wejściowej do testu repacku pamięci legacy. `current_line_archive_audit` traktował literal tej kontrolowanej próbki jak przypadkową starą referencję aktywnego runtime.
...
Zgodnie z zasadą projektu dodatkowa poprawka systemowa podnosi kanoniczną wersję do `16.0.5-runtime-turn-liveness-ci-hardening`.

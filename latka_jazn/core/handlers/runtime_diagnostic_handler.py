...
class RuntimeDiagnosticHandler:
    ...
    def handle(self, text: str, context: dict[str, Any] | None = None) -> RouteHandlerResult:
        ...
        return RouteHandlerResult(self.name,self.route,body,intent=intent,data={'startup_status':status},file_sources=[{'path':'latka_jazn/core/startup_contract.py'}],required_components=ctx.get('required_components',[]),satisfied_components=satisfied,confidence=0.82,source_origin_detail='runtime_diagnostic_handler/v15.1.0.3.89',truth_boundary='To jest status aktywnego folderu runtime, nie dowód stałego procesu po zakończeniu CLI.')

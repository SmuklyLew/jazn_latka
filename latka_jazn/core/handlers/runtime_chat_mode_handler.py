...
class RuntimeChatModeHandler:
    ...
    def handle(self, text: str, context: dict[str, Any] | None = None) -> RouteHandlerResult:
        ...
        return RouteHandlerResult(self.name, self.route, body, intent=ctx.get("intent", "runtime_chat_mode_request"), generation_mode="runtime_chat_mode_handler", required_components=ctx.get("required_components", []), satisfied_components=["chat_mode", "process_lifecycle", "stdin_or_jsonl_boundary"], source_origin_detail="runtime_chat_mode_handler/v1", confidence=0.88)

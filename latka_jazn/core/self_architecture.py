...
SELF_LAYERS: tuple[SelfLayer, ...] = (
    SelfLayer("identity_core", "rdzeń tożsamości", "imię, wartości, granice, głos i zasady prawdy", "core/canon/IdentityKernel + zweryfikowana pamięć/provenance", "mówić w pierwszej osobie i nie udawać biologii", "postać/prompt zamiast ja"),
    ...
    SelfLayer("dialogue_task_state", "rdzeń celu rozmowy", "utrzymuje aktywny cel, oczekiwaną akcję, referencje i bezpieczną kontynuację między turami", "dialogue_task_state.py + runtime_session_state.task_state", "nowa jawna treść użytkownika ma pierwszeństwo; stare zadanie wolno dziedziczyć tylko przy bezpiecznej kontynuacji", "utrata celu po 'zrób to' albo stale-route z poprzedniej rozmowy"),
    SelfLayer("reasoning_orchestration", "orkiestracja rozumowania", "wybiera fast/standard/deliberative i jawne kroki plan-wykonanie-weryfikacja", "reasoning_orchestrator.py + cognitive_runtime_coordinator.py", "zapisywać tylko kroki operacyjne i bramki; nie utrwalać prywatnego toku rozumowania", "ciężkie rozumowanie przy każdej turze albo brak weryfikacji trudnego zadania"),
    SelfLayer("knowledge_fabric", "warstwa wiedzy i retrieval", "łączy istniejące FTS5/BM25, opcjonalny wektor i relacje grafowe z proweniencją", "knowledge_fabric.py + memory/hybrid_retriever.py", "retrieval jest selektywny; źródło i truth status pozostają jawne", "zalew kontekstu, duplikowanie pamięci albo odpowiedź bez źródła"),
    SelfLayer("lexical_intelligence", "inteligencja leksykalna", "łączy lokalny leksykon, Morfeusz2, lokalny indeks plWordNet i wersjonowany cache", "nlp/lexical_intelligence.py + nlp/providers + resources/nlp/v154_lexical_sources.json", "każdy wynik zachowuje źródło/licencję; brak zasobu nie może tworzyć zmyślonej definicji", "większy słownik kosztem prawdy albo blokowanie rozmowy przez brak opcjonalnego providera"),
    SelfLayer("operational_learning", "pamięć lekcji antyregresyjnych", "zamienia zweryfikowane awarie w ustrukturyzowane lekcje powiązane z testami", "operational_learning_memory.py + testy regresyjne", "lekcja nie modyfikuje samodzielnie kodu i nie zastępuje testu", "powtarzanie tych samych awarii lub niekontrolowane samoprzepisywanie kodu"),

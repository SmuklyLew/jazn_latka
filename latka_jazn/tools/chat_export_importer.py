...
def _fast_source_info(path: Path, source_sha256: str) -> ExportSourceInfo:
    return ExportSourceInfo(
        path=str(path), source_name=path.name, source_kind=_source_kind(path),
        sha256=source_sha256, size_bytes=path.stat().st_size if path.is_file() else 0,
        conversations_member=None, html_member=None, crc_checked=False, crc_ok=True,
    )
...
class ChatExportImporter:
    ...
    def plan(self, source: str | Path, database: str | Path) -> ImportPlan:
        ...
        with ChatExportArchiveStore(database) as store:
            ...
            with ChatExportReader(source_path, verify_crc=True) as reader:
                ...
                return ImportPlan(source=reader.info, export_relation="new_export", conversations=plans)
        ...
        with ChatExportArchiveStore(database) as store:
            ...
            with ChatExportReader(source_path, verify_crc=True) as reader:
                ...
                return ImportResult(
                    ...
                    errors=errors,
                    warnings=([f"conflicts_recorded={writes.get('conflicts', 0)}"] if writes.get("conflicts", 0) else []),
                )
    ...
    def import_many(
        ...
        *,
        ...
        """Import each large export in a fresh Python process for deterministic memory reclamation."""
        import json
        import os
        import subprocess
        import sys
        ...
        for path in paths:
            command = [
                ...
                "--source", str(path), "--database", str(Path(database).expanduser().resolve()),
            ]
            if dry_run:

...
class JournalStore(Store):
    ...
    def import_reader(self, reader: JournalReader, dry_run: bool = False) -> dict[str, Any]:
        """Import journal records without materializing JSONL/NDJSON in memory.

        JSON documents keep their compatibility list representation, while JSONL is
        consumed through ``JournalReader.iter_items()``.  Source counters are updated
        after the streaming transaction completes.
        """
        source_row = self.con.execute(
            "SELECT source_id,entry_count,invalid_count FROM journal_sources WHERE sha256=?",
            (reader.sha256,),
        ).fetchone()
        if source_row and not dry_run:
            return {
                "ok": True,
                "status": "identical_source_duplicate",
                "source_id": source_row[0],
                "entries_seen": int(source_row[1] or 0),
                "invalid_entries": int(source_row[2] or 0),
                "inserted": 0,
                "updated_revisions": 0,
                "linked_existing": 0,
                "streaming_jsonl": reader.path.suffix.lower() in {".jsonl", ".ndjson"},
            }
        if dry_run:
            ...
            for item in reader.iter_items():
                ...
                row = self.con.execute(
                    "SELECT content_sha256 FROM journal_entries WHERE identity_key=?",
                    (item.identity,),
                ).fetchone()
        ...
        with self.transaction():
            ...
            for item in reader.iter_items():
                ...
                if row is None:
                    ...
                    self.con.execute(
                        "INSERT INTO journal_fts(rowid,text) VALUES(?,?)",
                    ...
                    if row["content_sha256"] == item.content_hash:
                        ...
                        if doc:
                            ...
                            self.con.execute(
                                "INSERT INTO journal_fts(journal_fts,rowid,text) VALUES('delete',?,?)",
                                (doc[0], old_text),
                            )
                            self.con.execute(
                                "INSERT INTO journal_fts(rowid,text) VALUES(?,?)",
                            ...
                            self.con.execute(
                                "UPDATE journal_fts_docs SET title=?,truth_status=?,event_time_start=? WHERE entry_id=?",
                        ...
                        updated += 1
                # These link/revision inserts are idempotent by their explicit keys.
                self.con.execute(
                    """INSERT INTO journal_entry_sources(entry_id,source_id,source_record_id,content_sha256,seen_at_utc)
                       VALUES(?,?,?,?,?) ON CONFLICT(entry_id,source_id,source_record_id) DO NOTHING""",
                    (entry_id, source_id, item.record_id, item.content_hash, current),
                )
                revision_id = uid("journal-revision", entry_id, revision, item.content_hash)
                existing_revision = self.con.execute(
                    "SELECT content_sha256 FROM journal_revisions WHERE revision_id=?", (revision_id,)
                ).fetchone()
                if existing_revision is None:
                    self.con.execute(
                        """INSERT INTO journal_revisions(revision_id,entry_id,revision,source_id,content_sha256,
                           previous_sha256,raw_json,created_at_utc) VALUES(?,?,?,?,?,?,?,?)""",
                        (revision_id, entry_id, revision, source_id, item.content_hash, previous,
                         canonical_json(item.raw), current),
                    )
                elif str(existing_revision[0]) != item.content_hash:
                    raise ValueError("journal revision key collision with different content")
            self.con.execute(
                "UPDATE journal_sources SET entry_count=?,invalid_count=? WHERE source_id=?",
                (entries_seen, reader.invalid, source_id),
            )
        return {
            "ok": True,
            "status": "imported",

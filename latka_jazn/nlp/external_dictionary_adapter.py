...
class ExternalDictionaryAdapter:
    """Kontrolowany adapter słownikowy poprzednia linia runtime.
    ...
    """
    def __init__(self, root: Path, *, allow_network: bool = True, user_agent: str = 'LatkaJazn/v1 (+local-runtime; network-nlp-sjp-lexical-bridge)', timeout_seconds: float = 4.0, max_retries: int = 0, cache_ttl_seconds: int = 604800) -> None:
    ...
    def normalize(self, term: str, lang: str = 'pl') -> NormalizedLexeme:
        return NormalizedLexeme(term=term, normalized=self._norm(term), language=lang, provider='external_dictionary_adapter/v1', confidence=0.60)

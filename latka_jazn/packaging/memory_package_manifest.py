...
from .memory_package_types import (
    MEMORY_FORMAT_VERSION, MEMORY_MANIFEST_SCHEMA_V1, MEMORY_MANIFEST_SCHEMA_V2,
...
def verify_memory_package_manifest(
    ...
    if schema not in {MEMORY_MANIFEST_SCHEMA_V1, MEMORY_MANIFEST_SCHEMA_V2}:
    ...
    if schema == MEMORY_MANIFEST_SCHEMA_V1:
        ...
        if int(payload.get("memory_format_version", -1)) != MEMORY_FORMAT_VERSION:
            errors.append({"code": "memory_format_version_unsupported", "actual": payload.get("memory_format_version")})
        try:
    ...
    return {
        ...
        "runtime_version_is_provenance_only": schema == MEMORY_MANIFEST_SCHEMA_V2 or not require_runtime_match,
